...
namespace Bacro
{
    ...
    {
        public PlayTest(Bacro main) : base(main)
        {
            EngeryFull = 525;
            EnergyOfPizza = 25;

            pics["도시맵"] = "757 571 10 7::4 3,5 3::221 56 54,220 56 54";
            //도시맵 = 622 488 178 138::7 7,7 13,2 10,12 3::64 72 55,68 75 60,84 82 65,68 75 59
            ...
            pics["맵 총독관저"] = "383 311 16 17::7 7,7 13,2 10,12 3::203 63 38,56 18 0,225 57 33,226 53 29";
        ...
        public override void PlayMacro(string cmd)
        {
            if (cmd == "연습")
            {
                new Thread(() => {
                    ...
                    LeftClick("맵 총독관저", null, 1000);
                    RightClick("맵 총독관저", 5000);

                    macro.KeyClick(null, "home", 1000);

                    var detect = TabAndDetectFront("보초", 10, 10, 50, 2000);

                    if (!detect)
                    {
                        Console.WriteLine("돌리고 다시 탭한다.");

                        macro.KeyLongClick("ctrl", "a", 200, 100);

                        detect = TabAndDetectFront("보초", 20, 10, 50, 2000);
                    }

                    if (detect)
                    {
                        if (!DetectTarget("들어가기", 200, 10, 20))
                        {
                            Console.WriteLine("들어가기 실패!!");
                            return;
                        }

                        LeftClick("들어가기", null, 3000);
                    }
        ...
        void sell(string place, bool bochungTime)
        {
            if (place == "도구점")
            {
                if (bochungTime)

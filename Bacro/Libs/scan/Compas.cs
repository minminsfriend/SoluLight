...
namespace Bacro
{
    public class Compas
    {
        ...
        public int DegShipHead = 0;
        public int DegDest = 0;
        public kvec CoordDest = new kvec(0, 0);
        kvec ShipGps = new kvec(0, 0);
        readonly kvec cenPas = kvec.Parse2d("63 44");
        ...
        internal void FeedImagePas(Bitmap theImage)
        {
            if (ImagePass != null)
            {
                ImagePass.Dispose();
        ...
        int scanCompasDeg()
        {
            ...
            for (int deg = 0; deg <= 359; deg++)
            {
                ...
                List<kvec> pixels = getYellowPixels(ImagePass, cenPas, arrow, area);
        ...
        internal kvec getShipGps()
        {
            return ShipGps.copy();
        }
        internal int DegToRotate()
        {
            DegDest = GetDegNorthOrigin();

            var degRot = DegDest - DegShipHead;
        ...
        int GetDegNorthOrigin()
        {
            kvec xhead = ShipGps.copy();
            kvec xtail = CoordDest.copy();
